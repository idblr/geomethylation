...
label(GEO$Age.at.surgery) <- 'Age at surgery'
...
GEO$Institution <- recode(GEO$Institution, 'Hopkins ' = 'Hopkins')
...
GEO$Histology <- recode(GEO$Histology, 'SQUAMOUS ' = 'SQUAMOUS')
...
# Race/Ethnicity
## Recode 'A' and 'H' as 'O'
GEO$Race <- recode(GEO$Race, 'A' = 'O', 'H' = 'O')
# table(GEO$Race, useNA = 'always')
# B    O   W
# 121  25  109

## Factorize (W as referent)
GEO$Race <- fct_relevel(GEO$Race, 'W', 'O', 'B')
table(GEO$Race)
# table(GEO$Race, useNA = 'always')
#  W    O    B    <NA>
#  109  25   121  0
...
label(GEO$NDIpw_tr) <- 'NDI (Powell-Wiley; MD & IL reference)'

GEO$NDIpw_tr_US <- as.numeric(GEO$NDIpw_tr_US)
label(GEO$NDIpw_tr_US) <- 'NDI (Powell-Wiley; US reference)'

GEO$NDIpw_qt <- as.factor(GEO$NDIpw_qt)
label(GEO$NDIpw_qt) <- 'NDI (Powell-Wiley; MD & IL reference)'
levels(GEO$NDIpw_qt) <- c(
  'Quintile 1',
  'Quintile 2',
  'Quintile 3',
  'Quintile 4',
...
label(GEO$NDIpw_qt.1) <- 'NDI (Powell-Wiley; MD & IL reference)'

GEO$NDIpw_qt_US <- as.factor(GEO$NDIpw_qt_US)
label(GEO$NDIpw_qt_US) <- 'NDI (Powell-Wiley; US reference)'
levels(GEO$NDIpw_qt_US) <- c(
  'Quintile 1',
  'Quintile 2',
  'Quintile 3',
  'Quintile 4',
...
# Exclude Small Cell Carcinoma (2024-07-10)
CANCER <- subset(CANCER, Classification != 'SCLC')
...
# 1-3  4    <NA>
# 89   95    0
# CrossTable(CANCER$NDImesser_qt_d, CANCER$Stage4)

# Dichotomize NDI (Messer; US referent)
# table(CANCER$NDImesser_qt_US)
# Quartile 1 Quartile 2 Quartile 3 Quartile 4
#         44         19         38         83
CANCER$NDImesser_qt_US_d <- recode(
  CANCER$NDImesser_qt_US.1,
  '1' = '1-3',
  '2' = '1-3',
  '3' = '1-3',
  '4' = '4'
...
# 1  2  3  4  5
# 40 23 19 40 60
CANCER$NDIpw_qt_US_d <- recode(
  CANCER$NDIpw_qt_US.1,
  '1' = '1-4',
  '2' = '1-4',
  '3' = '1-4',
  '4' = '1-4',
